const APP_VERSION='quantentunnel-investment-double-charge-fix-20260906';
const API='/api';
...
let investmentPurchaseInProgress=false;
async function invest(plan,amount,dailyRate){
  // Prevent accidental double-clicks/multiple requests from the same page.
  if(investmentPurchaseInProgress)return;
  investmentPurchaseInProgress=true;
  const buttons=document.querySelectorAll('#plans button');
  buttons.forEach(b=>{if(!b.disabled){b.disabled=true;b.dataset.originalText=b.textContent;b.textContent='PROCESSING…';}});
  try{
    await api('/investments',{method:'POST',body:JSON.stringify({plan,amount,dailyRate})});
    toast('Investment activated. Open Active Investments to start your 24-hour claim cycle.');
    setTimeout(()=>location.href='/active-investments.html',700);
  }catch(e){
    investmentPurchaseInProgress=false;
    buttons.forEach(b=>{b.disabled=false;if(b.dataset.originalText)b.textContent=b.dataset.originalText;});
    toast(e.message);
    // Refresh the plan state in case another request already activated it.
    loadPlans();
  }
}
